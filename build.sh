...
sync_source() {
    if [ $sync_android_scr ]; then
      ...
      printf "%s\n" "*********************************************"
      printf "%s\n\n" $($reset)
      repo sync -j$(nproc) --force-sync --no-tags --no-clone-bundle -c > /dev/null 2>&1
      printf "%s\n\n" $($cyan)
      printf "%s\n" "*********************************************"
      printf '%s\n' "Repo Sync Finished"
      printf "%s\n" "*********************************************"
...
build() {
    ...
    bash $(cat /tmp/loc)/progress.sh lock
    lunch "$product_scr"-userdebug > build.log 2>&1
    make -j$(nproc) $build_type_scr > build.log 2>&1
    bash $(cat /tmp/loc)/progress.sh unlock
}
